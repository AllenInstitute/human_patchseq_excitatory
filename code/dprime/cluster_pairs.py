...
rf_feat_importance = lambda clf: clf.feature_importances_


def pairwise_cluster_distances(clf, data, features, cluster_label, 
                              fit_pairwise=True, details=False, ranking=rf_feat_importance, #dprime_feat,
...
#         clf = clf(priors=np.ones(2)/2 if flat_prior else None)
    ...
    for i in range(1, n_clust):
        for j in range(i):
            #get data subset for these two clusters (i & j)
